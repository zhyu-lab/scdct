...
def save_dataset_ids(RNA_data, ATAC_data, train_id, val_id, test_id, fold_idx):
    ...
    }).to_csv(f'./paired_RNA_ATAC/train_ids.csv',
              index=False)

    pd.DataFrame({
        "val_ids": RNA_data.obs_names[val_id]
    }).to_csv(f'./paired_RNA_ATAC/val_ids.csv',
              index=False)

    pd.DataFrame({
        "test_ids": RNA_data.obs_names[test_id]
    }).to_csv(f'./paired_RNA_ATAC/test_ids.csv',
              index=False)

    pd.DataFrame({
        "train_ids": ATAC_data.obs_names[train_id]
    }).to_csv(f'./paired_RNA_ATAC/atac_train_ids.csv',
              index=False)

    pd.DataFrame({
        "val_ids": ATAC_data.obs_names[val_id]
    }).to_csv(f'./paired_RNA_ATAC/atac_val_ids.csv',
              index=False)

    pd.DataFrame({
        "test_ids": ATAC_data.obs_names[test_id]
    }).to_csv(f'./paired_RNA_ATAC/atac_test_ids.csv',
              index=False)
...
def save_model(rna_encoder, atac_encoder, rna_decoder, atac_decoder, RNA_input_dim, ATAC_input_dim):
    ...
    }, f"./paired_RNA_ATAC/model_info.pth")
...
def train_ed(rna_encoder, atac_encoder, rna_decoder, atac_decoder,
    ...
    for epoch in range(R2R_train_epoch):
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(rna_encoder.state_dict(), f'./paired_RNA_ATAC/best_rna_encoder.pth')
            torch.save(rna_decoder.state_dict(), f'./paired_RNA_ATAC/best_rna_decoder.pth')
    ...
    rna_encoder.load_state_dict(torch.load(f'./paired_RNA_ATAC/best_rna_encoder.pth'))
    rna_decoder.load_state_dict(torch.load(f'./paired_RNA_ATAC/best_rna_decoder.pth'))
    ...
    for epoch in range(A2A_train_epoch):
        ...
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            patience_counter = 0
            torch.save(atac_encoder.state_dict(), f'./paired_RNA_ATAC/best_atac_encoder.pth')
            torch.save(atac_decoder.state_dict(), f'./paired_RNA_ATAC/best_atac_decoder.pth')
            print("Validation loss decreased, saving model.")
        else:
            patience_counter += 1
            print(f"Validation loss did not improve for {patience_counter} epochs.")
    ...
    atac_encoder.load_state_dict(torch.load(f'./paired_RNA_ATAC/best_atac_encoder.pth'))
    atac_decoder.load_state_dict(torch.load(f'./paired_RNA_ATAC/best_atac_decoder.pth'))
