...
if __name__ == "__main__":
    ...
    rna_train_loader, rna_val_loader, rna_test_loader, atac_train_loader, atac_val_loader, atac_test_loader = create_data_loaders(
        RNA_data, ATAC_data)

    rna_gen = get_model().to(device)  # noise prediction UNet for RNA
    atac_gen = get_model().to(device)  # noise prediction UNet for ATAC
    rna_encoder, atac_encoder, rna_decoder, atac_decoder = get_encoder_decoder(RNA_input_dim, ATAC_input_dim,
                                                                               chrom_list)
