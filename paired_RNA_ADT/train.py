...
if __name__ == "__main__":
    ...
    adt_gen = get_model().to(device)  # noise prediction UNet for ATAC
    ...
    adt_encoder = adt_encoder.to(device)
    rna_decoder = rna_decoder.to(device)
    adt_decoder = adt_decoder.to(device)

    optim_genA = Adam(rna_gen.parameters(), lr=configs.GENA_LR)
    optim_genB = Adam(adt_gen.parameters(), lr=configs.GENB_LR)

    optimizer_rna = Adam(list(rna_encoder.parameters()) + list(rna_decoder.parameters()),
                             lr=configs.RNA_LR)

    optimizer_adt = Adam(list(adt_encoder.parameters()) + list(adt_decoder.parameters()),
                             lr=configs.ADT_LR)

    optimizer_ae = Adam(list(rna_encoder.parameters()) + list(rna_decoder.parameters()) +
                        list(adt_encoder.parameters()) + list(adt_decoder.parameters()),
                          lr=configs.AE_LR)

    models = [rna_gen, adt_gen, rna_encoder, adt_encoder, rna_decoder, adt_decoder]
    optimizers = [optim_genA, optim_genB, optimizer_rna, optimizer_adt, optimizer_ae]


    best_models,diff_train_losses, diff_val_losses = train_with_early_stopping(
        models, optimizers,
        data_loaderA_train=rna_train_loader,
        data_loaderB_train=adt_train_loader,
        data_loaderA_val=rna_val_loader,
        data_loaderB_val=adt_val_loader,
        data_loaderA_test=rna_test_loader,
        data_loaderB_test=adt_test_loader,
